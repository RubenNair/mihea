...
double DT5BlockFullRotEllipWrongExponent_t::getLowerRangeBound( int dimension )
{
	return( -1e308 );
}
		
double DT5BlockFullRotEllipWrongExponent_t::getUpperRangeBound( int dimension )
{
	return( 1e308 );
}
		
int DT5BlockFullRotEllipWrongExponent_t::getNumberOfSubfunctions()
{
	return number_of_variables;
}
		
vec_t<int> DT5BlockFullRotEllipWrongExponent_t::inputsToSubfunction( int subfunction_index )
{
	vec_t<int> vec;
...
double DT5BlockFullRotEllipWrongExponent_t::continuous_subfunction(int subfunction_index, vec_t<int> &variables, vec_t<double> &c_variables)
{
    ...
    double res = 0.0;
    for(int i = 0; i < k; i++) {
        res += pow(10, 6 * ((double)(i+subfunction_index*k) / (double)(number_of_c_variables - 1))) * pow(c_variables[i+subfunction_index*k] - ellipsoid_centre[i], 2);
    }
    return res;
}
...
void DT5BlockFullRotEllipWrongExponent_t::evaluationFunction( solution_t<int> *solution )
{
    ...
    // Use variables rotated along entire c_variables vector (instead of in blocks)
	...
	for( int i = 0; i < solution_mix->getNumberOfVariables() / k; i++ )
	{
        ...
        double fsub_ellipse = continuous_subfunction(i, solution_mix->variables, rotated_variables);
...
double DT5BlockFullRotEllipWrongExponent_t::objectiveFunction( int objective_index, vec_t<double> &fitness_buffers )
{
...
double **DT5BlockFullRotEllipWrongExponent_t::initializeObjectiveRotationMatrix( double rotation_angle, int rotation_block_size )
{
...
double *DT5BlockFullRotEllipWrongExponent_t::rotateVariables( double *variables, int num_variables, double **rotation_matrix )
{
